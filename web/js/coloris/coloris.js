 ...
 *
 * Modified for ComfyUI-komlevv-tweaks:
 * - reduced to a lean desktop/browser implementation for ComfyUI
 * - removed virtual instances support
 * - removed custom parent container support
 * - removed input wrapping / preview field support
 * - removed touch event support
 * - removed legacy browser fallbacks
 */
...
((window, document, Math) => {
  ...
  function getColorFormatFromStr(str = "") {
    const format = String(str).substring(0, 3).toLowerCase();
  ...
  function strToRGBA(str) {
    ...
    ctx.fillStyle = "#000";
    ctx.fillStyle = str || "#000";
    let match = regex.exec(ctx.fillStyle);
  ...
  function updateMarkerA11yLabel(saturation, value) {
    ...
    label = label.replace("{s}", saturation.toFixed(1) * 1);
    label = label.replace("{v}", value.toFixed(1) * 1);
  ...
  function updateColor(rgba = {}, hsva = {}) {
    ...
    const formatInput = document.querySelector(`.clr-format [value="${format}"]`);
  ...
  function pickColor(color = colorValue.value) {
    if (currentEl) {
  ...
  function moveMarker(event) {
    ...
    const y = pointer.pageY - colorAreaDims.y;
  ...
  function bindFields(selector) {
    ...
    if (Array.isArray(selector)) {
      selector.forEach((field) => addListener(field, "click", openPicker));
    } else {
      addListener(document, "click", selector, openPicker);
    }
  }

  function rebuildSwatches(swatches) {
    ...
    if (swatches.length) swatchesContainer.appendChild(swatchGroup);
  }
    ...
    for (const key in options) {
      switch (key) {
        ...
        case "theme":
          ...
          if (settings.inline) updatePickerPosition();
          break;
        case "margin":
          options.margin *= 1;
        ...
        case "formatToggle":
          ...
          if (settings.formatToggle) settings.format = "auto";
          break;
        case "swatches":
          if (Array.isArray(options.swatches)) {
            rebuildSwatches(options.swatches);
        ...
        case "a11y": {
          ...
          if (update) {
            getEl("clr-open-label").innerHTML = settings.a11y.open;
            getEl("clr-swatch-label").innerHTML = settings.a11y.swatch;
  ...
  function init() {
    ...
    addListener(colorValue, "change", () => {
      ...
      if (currentEl || settings.inline) {
        const color = value === "" ? value : (setColorFromStr(value), value);
        pickColor(color);
      }
    });
