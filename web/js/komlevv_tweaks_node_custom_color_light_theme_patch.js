...
const EXTENSION_ID = "komlevv.tweaks.nodeCustomColorLightThemePatch";
...
function removeExistingCustomEntries(menuElement) {
  const entries = menuElement?.querySelectorAll?.(".litemenu-entry");
...
app.registerExtension({
  ...
  setup() {
    ...
    const previousOnMenuNodeColors = LGraphCanvas.onMenuNodeColors;
    LGraphCanvas.onMenuNodeColors = function (value, options, e, menu, node) {
      const result = previousOnMenuNodeColors.apply(this, arguments);
      ...
      requestAnimationFrame(() => {
        ...
        for (let i = menus.length - 1; i >= 0; i--) {
          ...
          removeExistingCustomEntries(targetMenu);
